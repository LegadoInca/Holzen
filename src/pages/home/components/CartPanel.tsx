...
const CartPanel = ({ isOpen, onClose, items, onRemove, onQtyChange }: CartPanelProps) => {
  ...
  const sendWhatsApp = () => {
    const lines = items.map(
      (i) => `• ${i.name} x${i.qty} (${i.priceLabel})`
    );
    const text = `Hola, me interesa el café HOLZEN. Mi pedido:\n${lines.join('\n')}\nTotal estimado: $${total.toFixed(2)}`;
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-gold/10">
          <h2 className="font-serif text-xl text-cream">Tu Pedido</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-cream/40">
              <i className="ri-cup-line text-5xl" />
            ...
            <ul className="flex flex-col gap-4">
              {items.map((item) => (
                <li
                  key={item.id}
                  className="flex gap-4 items-center bg-coffee-700/40 rounded-lg p-3"
                >
                  <img
                    src={item.image}
                    alt={item.name}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-cream text-sm font-medium font-serif truncate">
                      {item.name}
                    </p>
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <button
                        ...
                        className="w-6 h-6 flex items-center justify-center border border-gold/30 text-gold rounded cursor-pointer hover:bg-gold/10 text-sm"
                      >
                        –
                      </button>
                  ...
                  <button
                    ...
                    className="w-7 h-7 flex items-center justify-center cursor-pointer text-cream/30 hover:text-cream/70 transition-colors"
                  >
        ...
        {items.length > 0 && (
          <div className="px-6 py-5 border-t border-gold/10">
            <div className="flex justify-between items-center mb-4">
              <span className="text-cream/70 text-sm font-sans">Total estimado</span>
            ...
            <p className="text-cream/40 text-xs text-center mt-3 font-sans leading-relaxed">
              Te contactaremos para confirmar disponibilidad, envío y método de pago.
            </p>
          </div>
        )}
      </div>
